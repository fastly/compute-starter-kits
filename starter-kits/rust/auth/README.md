...
### A note on `cargo audit`

Running `cargo audit` against this project may report [RUSTSEC-2023-0071](https://rustsec.org/advisories/RUSTSEC-2023-0071) (the "Marvin Attack") in the `rsa` crate, a transitive dependency of `jwt-simple`. There is no fixed release of `rsa` available for this advisory. It only affects RSA *private-key* operations (decryption/signing); this starter kit only ever verifies JWT signatures using a public key (see `src/jwt.rs`), so the vulnerable code path is never exercised here.
