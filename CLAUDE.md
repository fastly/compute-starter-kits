...
- **Kits that declare no slug still have a docs URL**, constructed by `derivedSlug()` as `compute-starter-kit-<lang>-<name>`. Those constructed values take part in the global uniqueness check, or a newly declared `alt_slug` could silently shadow another kit's existing page. The documentation site has its own copy of this fallback (`resolveKitSlug`, in `fastly/customer-documentation`) — the two must stay identical, or this repo guards a string nobody serves. Keeping it a single expression with no special cases is what makes that feasible; a kit whose URL doesn't fit the shape declares `slug` explicitly instead, which is the only exception mechanism.
  - The four `typescript-*` kits are the live example: their URLs omit the `javascript` segment, and that is expressed purely by each of them declaring `slug`, not by a branch in `derivedSlug()`. Don't "tidy up" those declarations — they are load-bearing, as are `compute-js-auth` and `compute-rust-auth`, which predate the `compute-starter-kit-*` convention.
